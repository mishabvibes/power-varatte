...
interface DonationType {
  _id: string; // Mongoose ID as string after .lean()
  name?: string;
  email?: string;
  phone?: string;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const donations = await Donation.find({
      _id: { $in: donationIds }
    }).lean() as unknown as DonationType[];
...
async function generateCSVReceipt(donations: DonationType[], updateStatus: boolean = false) {
  try {
    ...
    const csvData = donations.map(donation => {
      ...
      return {
        ...
        'Amount': `₹${parseFloat(String(donation.amount)).toLocaleString('en-IN')}`,
...
async function generatePDFReceipt(donations: DonationType[], updateStatus: boolean = false) {
  let browser;
  try {
    // Read logo file
    let logoDataUri = '';
...
async function generateImageReceipt(donations: DonationType[], updateStatus: boolean = false) {
  let browser;
  try {
    // Read logo file
    let logoDataUri = '';
...
function formatDonationToHTML(donation: DonationType, logoDataUri: string = ''): string {
  // Format date
  const receiptDate = donation.createdAt ? new Date(donation.createdAt).toLocaleDateString() : 'N/A';
  ...
  // Format amount
  const amount = `₹${parseFloat(String(donation.amount)).toLocaleString('en-IN')}`;
